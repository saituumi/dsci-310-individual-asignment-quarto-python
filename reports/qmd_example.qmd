...
![](../results/horse_pops_plot.png){#fig-hs1}

@fig-hs1: Horse populations for all provinces in Canada from 1906 - 1972.

We can see from @fig-hs1 that Ontario, Saskatchewan and Alberta 
...
We present the standard deviations in @tbl-hst.
...
Additionally, note that in @table-hst we consider the sample standard deviation of the number of horses during the same time span as @fig-hs1.

![Horse populations for the province with the largest standard deviation](../results/horse_pops_plot_largest_sd.png){#fig-hs2 width=60%}

@fig-hs2: Horse populations for the province with the largest standard deviation


```{python}
from generates_figure import main
largest_sd_prov = horses_sd.iloc[0]['Province']
```

In @fig-hs2 we zoom in and look at the province of '{python} largest_sd_prov', which had the largest spread of values in terms of standard deviation.
